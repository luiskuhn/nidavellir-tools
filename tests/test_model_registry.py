...
import nidavellir_tools.model_package_registry as registry
...
def make_package(path: Path) -> Path:
    ...
    rdf = {
        ...
        "weights": {
            "torchscript": {"source": "model.ts", "sha256": registry._digest(path / "model.ts")}
        },
    }
...
def test_loads_rdf_state_dict_with_packaged_architecture(tmp_path):
    ...
    rdf = {
        ...
        "weights": {
            "pytorch_state_dict": {
                "source": "weights.pt",
                "sha256": registry._digest(weights),
                "architecture": {
                    "source": "architecture.py",
                    "sha256": registry._digest(architecture),
                    "callable": "Network",
                    "kwargs": {"features": 2},
                },
            }
...
def test_exported_child_preserves_specification_and_reloads_as_parent(tmp_path):
    ...
    rdf = {
        ...
        "documentation": {
            "source": "README.md",
            "sha256": registry._digest(card),
        },
        "inputs": [
            {
                "test_tensor": {
                    "source": "test-input.npy",
                    "sha256": registry._digest(test_input),
                }
            }
        ],
        "outputs": [
            {
                "test_tensor": {
                    ...
                    "sha256": registry._digest(test_output),
                }
            }
        ],
        "weights": {
    ...
    torch.save(
        {"state_dict": {f"model.{key}": value for key, value in child_model.state_dict().items()}},
        child_checkpoint,
    )
    ...
    child = registry.export_child_package(
        ...
        tmp_path / "child",
        version="2",
        parent_identifier="hf://owner/parent@commit",
