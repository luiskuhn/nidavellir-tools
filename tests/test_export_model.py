...
import nidavellir_tools.build_model_package as builder

PACKAGE_ROOT = Path(builder.__file__).parent


def test_default_model_card_has_validation_section():
    card = (PACKAGE_ROOT / "model-card-template.md").read_text(encoding="utf-8")
...
def test_packaging_reference_is_valid_yaml_and_has_required_contract_sections():
    reference = yaml.safe_load(
        (PACKAGE_ROOT / "examples" / "model-package.example.yaml").read_text()
    )
...
def test_builds_domain_independent_repository_package(tmp_path, monkeypatch, validation_mode):
        ...
        yaml.safe_dump(
            {
                ...
                "type": "model",
                "name": "Generic vision model",
                "version": "1.0.0",
                "description": "No domain assumptions.",
    ...
    torch.save(
        {"state_dict": {f"network.{key}": value for key, value in network.state_dict().items()}},
        checkpoint,
    )
    ...
    assert rdf["inputs"][0]["sample_tensor"]["sha256"] == builder._digest(
        package / "sample-input.tif"
    )
    assert rdf["outputs"][0]["sample_tensor"]["sha256"] == builder._digest(
        package / "sample-output.tif"
    )
    ...
    with zipfile.ZipFile(archive) as package_zip:
        ...
        assert tifffile.imread(io.BytesIO(package_zip.read("sample-output.tif"))).shape == (
            2,
            2,
            3,
            4,
