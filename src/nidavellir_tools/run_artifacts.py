...
def prepare_model_package_artifacts(
    ...
    overwrite = bool(parameters.get("overwrite_model_package", parameters.get("overwrite", False)))
    ...
    if model_card_path is None:
        shutil.copy2(Path(__file__).with_name("model-card-template.md"), output_dir / "README.md")
    else:
