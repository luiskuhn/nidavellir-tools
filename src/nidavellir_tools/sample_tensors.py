...
def create_sample_tensor(
    ...
    *,
    ...
    if channel_policy not in CHANNEL_POLICIES:
        raise ValueError(f"sample channel policy must be one of {', '.join(CHANNEL_POLICIES)}")
    ...
    if layout != "auto" and resolved != layout.upper():
        raise ValueError(f"explicit layout {layout.upper()} does not match RDF axes {resolved}")

    batch_axis = resolved.index("B")
    batch_size = array.shape[batch_axis]
    if batch_index < 0 or batch_index >= batch_size:
        raise IndexError(f"sample batch index {batch_index} is outside [0, {batch_size})")
