...
def _rdf_root(path: Path) -> Path:
    candidates = (
        [path]
        if (path / "rdf.yaml").is_file()
        else [item.parent for item in path.rglob("rdf.yaml")]
    )
    if len(candidates) != 1:
...
def stage(
    source: str, destination: Path, *, revision: str | None = None, overwrite: bool = False
) -> Path:
...
def load_package(package: Path, *, representation: str = "auto") -> LoadedModelPackage:
    ...
    if representation in {"auto", "torchscript"} and "torchscript" in weights:
        ...
        constructor = _resolve_callable(architecture["callable"], root / source if source else None)
        model = constructor(**architecture.get("kwargs", {}))
        state = torch.load(root / descriptor["source"], map_location="cpu", weights_only=True)
...
def export_child_package(
    ...
    *,
    ...
    parent_provenance = (
        json.loads(old_provenance.read_text(encoding="utf-8")) if old_provenance.is_file() else {}
    )
    provenance = {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    load_parser.add_argument(
        "--representation", default="auto", choices=["auto", "torchscript", "pytorch_state_dict"]
    )
    load_parser.add_argument(
    ...
    if args.command == "stage":
        ...
        if args.weights_output:
            if loaded.representation != "pytorch_state_dict":
                raise ValueError("--weights-output requires --representation pytorch_state_dict")
        ...
        metadata = {
            ...
            "weights_sha256": (loaded.rdf["weights"][loaded.representation].get("sha256")),
            "rdf": loaded.rdf,
